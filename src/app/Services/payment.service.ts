...
export class PaymentService {
  ...
  createPaymentWithPatient(patientId: number, payment: Payment): Observable<Payment> {
    return this.http.post<Payment>(`${this.baseUrl}/create-with-patient/${patientId}`, payment);
  }
  ...
  getTotalPaymentByPatientId(patientId: number): Observable<number> {
    return this.http.get<number>(`${this.baseUrl}/total-by-patient/${patientId}`);
  }
  ...
  getTotalPaymentsByPatient(): Observable<Map<string, Object>[]> {
    return this.http.get<Map<string, Object>[]>(`${this.baseUrl}/total-by-patient`);
  }

  getPaymentsForDay(date: string): Observable<Payment[]> {
    return this.http.get<Payment[]>(`${this.baseUrl}/day/${date}`);
  }
